...
def but_click():
    select_options= ''
    if asia.get():
        select_options += chkbut_asia.cget('text')
    if america.get():
        select_options += chkbut_america.cget('text')
    if europe.get():
        select_options += chkbut_europe.cget('text')
    if aferica.get():
        select_options += chkbut_aferica.cget('text')
    lab_result.config(text=select_options)
win = tk.Tk()
...
but=tk.Button(win,text='確定',command=but_click,font=default_font,padx=16)
